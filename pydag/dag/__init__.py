...
class DAGNode(object):
    def __init__(self, block, edges=None):
        self.block = block
        self.edges = []
    # node_id, parent_hash, and own hash are within block - need to update add_edge to add parent
...
class DAG(object):
    ...
    def leaf_selection(self, block, gnode_id):
        x1 = block.coords[1]
        y1 = block.coords[2]
    ...
    def leaf_selection2(self, block):
        return self.ind_nodes()[0].block.node_id

    def merkle_hash(self, block):
        hashlist = []
        for predecessor in self.all_downstreams2(block.node_id):
            hashlist.append(self.graph[predecessor].block.__hash__())
        return hashlist
